...
class PollingUnit(models.Model):
    ...
    def ward(self):
        from_ward = None
        if self.ward_id:
            from_ward  =Ward.objects.filter(ward_id = self.ward_id).first()
        return from_ward
    
    @property
    def result(self):
        partties = AnnouncedPuResults.objects.filter(polling_unit_uniqueid = self.polling_unit_id)
        return partties

    def __str__(self,*args,**kwargs):
        return self.polling_unit_name
    def get_absolute_url(self):
        return reverse('detail',kwargs={'pk':self.uniqueid})